...
export default function IncidentStatusSelect({
  incident,
  disabled = false,
    ...
    >
      ...
      >
        ...
        <p className="mt-5 text-sm leading-6 text-stone-500">
          ...
          <span className="font-semibold text-[#4B3932]">{statusLabels[incident.status]}</span> to{" "}
          <span className="font-semibold text-[#4B3932]">{statusLabels[pendingStatus]}</span>?
        </p>
  ...
  `}
      ...
      {pendingStatus && createPortal(modal, document.body)}
    </>
  );
}
